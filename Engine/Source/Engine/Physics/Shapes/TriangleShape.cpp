...
TriangleShape::TriangleShape(glm::vec3 a, glm::vec3 b, glm::vec3 c)
{
	...
	minX = minY = minZ = kInfinity;
	maxX = maxY = maxZ = -kInfinity;
	...
	mBoundingBox = BoundingBox(minX, minY, minZ, maxX, maxY, maxZ);
}

bool TriangleShape::Intersects(IPhysicsShape *other, CollisionData *collision)
{
	return false;
}

bool TriangleShape::RayIntersects(Ray ray, glm::vec3 *outPoint)
{
